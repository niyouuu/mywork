...
# 使用函数
video_path = "your_video_path.mp4"  # 输入你的视频路径
...
def cropPic():
    # file_count=0
    # folder_path="analyze/temp_cropped"
    ...
    #     print("文件index",i)
    cmd = "python ../Talking-Face_PC-AVS-main/scripts/align_01.py --folder_path ../PyQt-Sqlite-Project-CURD-master/analyze/temp_cropped"
...
# 21,23,27,35,37,39
# 22,34,38,40
pa_no_set=["36","37","38","39","40","41","43","44","45","47","49","52","54","55","56","57","58","60"]
psnr_arr=[]
final_psnr_arr=[]
ssim_arr=[]
final_ssim_arr=[]
for j in range(len(pa_no_set)):
    pa_no=pa_no_set[j]
    ...
    for i in range(n):
        ...
        print("trainfile",train_path)
        generate_path=generate_files[i]
        print("generatepath",generate_path)
        cropped_path="analyze/"+pa_no+"/cropped_train/"+str(i)+".mp4"
        temp_cropped_path="analyze/temp_cropped"
        video_to_frames(train_path,temp_cropped_path)
        cropPic()
        input_dir = temp_cropped_path  # 输入目录
        output_video = cropped_path  # 输出视频路径
        fps = 24.0  # 视频帧率
        frames_to_video(input_dir+"_cropped", output_video, fps)
        temp_avg_psnr, temp_avg_ssim = calculate_psnr_ssim(cropped_path, generate_path)
        psnr_arr.append(temp_avg_psnr)
        ssim_arr.append(temp_avg_ssim)
        print(pa_no+"ssim",temp_avg_ssim)
        print(pa_no+"psnr",temp_avg_psnr)
        delete_files_in_folder(temp_cropped_path)  # 删除存储temp_cropped_images
        delete_files_in_folder(temp_cropped_path+"_cropped")  # 删除存储temp_cropped_images
    final_avg_psnr=np.mean(temp_avg_psnr)
    final_avg_ssim=np.mean(temp_avg_ssim)
    print(pa_no+"的平均psnr:", final_avg_psnr)
    print(pa_no+"的平均ssim:", final_avg_ssim)
    psnr_arr.clear()
    ssim_arr.clear()
