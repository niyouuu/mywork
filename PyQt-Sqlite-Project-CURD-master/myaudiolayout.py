...
class AudioRecorder(QWidget):
    pagestate_clicked = pyqtSignal(bool)
    def __init__(self, pa_no, train_str_no, trial_no, parent=None):
        super().__init__(parent)
        self.pa_no = pa_no
        self.train_str_no = train_str_no
        self.trial_no = trial_no
        self.folder_path = "pa_train/pa_" + str(self.pa_no)
        self.setFilePath(self.trial_no,self.train_str_no)
        self.initUI()
    ...
    def start_recording(self):
        try:
            ...
            self.play_button.setEnabled(False)
            self.on_pagestate_clicked(False)
    ...
    def stop_recording(self):
        try:
            if self.audio_input is not None:
                ...
                self.on_pagestate_clicked(True)
        except Exception as e:
            print(f'Error: {e}')
    ...
    def save_recording(self):
        try:
            ...
            if not os.path.exists(self.folder_path):
                os.makedirs(self.folder_path)  # 如果文件夹不存在，则创建文件夹
            if self.file_path:
    ...
    def setAuBtnsState(self,b_value):
        try:
            self.stop_button.setEnabled(b_value)
            self.play_button.setEnabled(b_value)
        except Exception as e:
            print(f'Error: {e}')

    def on_pagestate_clicked(self,value):
        self.pagestate_clicked.emit(value)

    def setTrainStr(self,train_str_no):
        self.train_str_no=train_str_no
    def setTrialNo(self,trial_no):
        self.trial_no = trial_no
    def setFilePath(self,trial_no,train_str_no):
        self.file_path = self.folder_path + "/trial" + str(trial_no) + "_" + str(train_str_no) + ".mp3"



if __name__ == '__main__':
    app = QApplication(sys.argv)
    recorder = AudioRecorder("1","w2","0")
    recorder.show()
    sys.exit(app.exec_())
